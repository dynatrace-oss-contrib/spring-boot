...
* Use Dynatrace Summary instruments: In some cases the Micrometer Dynatrace registry created metrics that were rejected. Since Micrometer 1.9.x, this should be fixed. Setting this toggle to false forces Micrometer to fall back to the behavior that was the default before 1.9.x. It should only be used if encountering problems while migrating from 1.8.x to 1.9.x.
