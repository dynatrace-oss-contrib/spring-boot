...
==== Dynatrace
Dynatrace offers two metrics ingest APIs:

===== API v1
The Dynatrace API v1 registry pushes metrics to the configured URI periodically.
...
===== API v2
The API v2 can be used int two ways: If a local OneAgent is running on the machine, it is enough to tell the exporter to use v2 and metrics will be automatically exported to the https://www.dynatrace.com/support/help/how-to-use-dynatrace/metrics/metric-ingestion/ingestion-methods/local-api/[local OneAgent endpoint], which forwards them to the Dynatrace backend:
...
If no local OneAgent is running, the token and URI are required. The https://www.dynatrace.com/support/help/dynatrace-api/basics/dynatrace-api-authentication/[API token] must have the `metrics.ingest` permission set.
...
When using the Dynatrace v2 API, new optional features are available:

  * Metric key prefix: set a prefix that will be prepended to all exported metric names.
...
----
	management:
	  metrics:
	    export:
	      dynatrace:
            # ... token and uri or blank for OneAgent export, and api-version: "v2" ...
